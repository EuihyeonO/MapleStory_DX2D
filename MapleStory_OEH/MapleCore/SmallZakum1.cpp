...
void SmallZakum1::Update(float _DeltaTime)
{
	StatusUpdate(_DeltaTime);
	AttackRangeCheck();
}

void SmallZakum1::Render(float _DeltaTime)
{

}

void SmallZakum1::Spawn(float _DeltaTime)
{
...
void SmallZakum1::AttackRangeCheck()
{
...
void SmallZakum1::SetAnimation()
{
	BasicRender->CreateAnimation({ .AnimationName = "FLY",.SpriteName = "SmallZakum1Fly",.FrameInter = 0.1f ,.Loop = true,.ScaleToTexture = true });
	BasicRender->CreateAnimation({ .AnimationName = "HIT",.SpriteName = "SmallZakum1Hit",.FrameInter = 0.6f ,.Loop = false,.ScaleToTexture = true });
	BasicRender->CreateAnimation({ .AnimationName = "DEATH",.SpriteName = "SmallZakum1Death",.FrameInter = 0.15f ,.Loop = false,.ScaleToTexture = true });
...
void SmallZakum1::Fly(float _DeltaTime)
{
...
void SmallZakum1::Hit(int _Damage, bool _isRealAttack)
{
	Hp -= _Damage;
...
void SmallZakum1::MonsterDeath(float _DeltaTime)
{
...
void SmallZakum1::Attack()
{
	if(isAttack == false)
	{
		std::weak_ptr<GameEngineSpriteRenderer> NewEff = CreateComponent<GameEngineSpriteRenderer>();
		NewEff.lock()->CreateAnimation({ .AnimationName = "Attack",.SpriteName = "SmallZakum1Attack",.FrameInter = 0.1f,.Loop = false,.ScaleToTexture = true });
		NewEff.lock()->SetAnimationUpdateEvent("Attack", 15, [NewEff, this] {if (NewEff.lock()->IsAnimationEnd() == true) { Player::GetCurPlayer()->Hit(Att); NewEff.lock()->Death(); isAttack = false; MoveType = "FLY"; }});
